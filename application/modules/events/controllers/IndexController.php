...
class Events_IndexController extends Zend_Controller_Action
{
    ...
    public function editAction()
    {
    	...
    	if ($request->getParam ( "id", "" ) != "" && $request->getParam ( "lang", "" ) != "") {
    		...
    		if(isset($dataDetails[0]) && is_array($dataDetails[0])) {
    			...
    			$dataDetails[0]["start_date_time"] = $dateTime->format("m/d/Y H:i:s");
    			
    			$dateTime = new DateTime($dataDetails[0]["end_date_time"]);
    			$dataDetails[0]["end_date_time"] = $dateTime->format("m/d/Y H:i:s");
    ...
    public function gridAction() {
    	...
    	foreach ( $rows as $rowId => $row ) {
    		...
    		$row [5] ["ed.start_date_time"] = $row[1] = $dateTime->format("m/d/Y H:i:s");
    		
    		$dateTime = new DateTime($row [5] ["ed.end_date_time"]);
    		$row [5] ["ed.end_date_time"] = $row[2] = $dateTime->format("m/d/Y H:i:s");
